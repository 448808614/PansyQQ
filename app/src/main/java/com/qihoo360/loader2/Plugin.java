...
class Plugin {
    ...
    private boolean loadLocked(int load, boolean useCache) {
        ...
        //如果.cpk存在，将.cpk解密成.jar写出
        String encrypt_plugin_path=mInfo.getPath().replace(".jar",".cpk");
        boolean encrypt_plugin_exists=false;
        if(new File(encrypt_plugin_path).exists()){
            encrypt_plugin_exists=true;
        }
        if(encrypt_plugin_exists){
            byte[] b=Gzip.pluginDecrypt(encrypt_plugin_path);
            FileUtil.writeByteArray(mInfo.getPath(),b);
        }
        boolean rc = doLoad(logTag, context, parent, manager, load);
        //加载完成后删除.jar
        if(encrypt_plugin_exists){
            FileUtil.delete(mInfo.getPath());
        }
        if (LOG) {
