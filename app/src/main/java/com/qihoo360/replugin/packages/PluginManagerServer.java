...
public class PluginManagerServer {
    ...
    private boolean copyOrMoveApk(String path, PluginInfo instPli) {
        ...
        //把.apk加密成.cpk写到app_p_a,删除apk
        byte[] b=Gzip.pluginEncrypt(srcFile.getAbsolutePath());
        String encrypt_plugin_path=newFile.getAbsolutePath().replace(".jar",".cpk");
        FileUtil.writeByteArray(encrypt_plugin_path,b);
