...
* <code> * 
*      ------- 消息填充算法 ----------- 
*      network_security_config = (明文长度 + 10) mod 8
*      if(network_security_config 不等于 0) network_security_config = 8 - network_security_config;
*      b = 随机数 &amp; 0xF8 | network_security_config;              这个的作用是把a的值保存了下来
*      plain[0] = b;                     然后把b做为明文的第0个字节，这样第0个字节就保存了a的信息，这个信息在解密时就要用来找到真正明文的起始位置
*      plain[1 至 network_security_config+2] = 随机数 &amp; 0xFF;    这里用随机数填充明文的第1到第a+2个字节
*      plain[network_security_config+3 至 network_security_config+3+明文长度-1] = 明文; 从a+3字节开始才是真正的明文
*      plain[network_security_config+3+明文长度, 最后] = 0;       在最后，填充0，填充到总长度为8的整数为止。到此为止，结束了，这就是最后得到的要加密的明文内容
*      ------- 消息填充算法 ------------ *   
* </code>
* </pre> 
*/
 public class Tea {
    ...
    // 随机数对象
    private static Random random = TeaUtil.random();
    // 字节输出流
  ...
  public Tea() {
    ...
    public byte[] xdecrypt(byte[] in,byte[] k){
        loop=0x20;
        delta=0x57E89147;
        return decrypt(in,k);
    }
    ...
    public byte[] xencrypt(byte[] in,byte[] k){
        loop=0x20;
        delta=0x57E89147;
        return encrypt(in,k);
    }
    ...
    private byte[] encipher(byte[] in) {
        ...
        long y = TeaUtil.getUnsignedInt(in, 0, 4);
        long z = TeaUtil.getUnsignedInt(in, 4, 4);
        long a = TeaUtil.getUnsignedInt(key, 0, 4);
        long b = TeaUtil.getUnsignedInt(key, 4, 4);
        long c = TeaUtil.getUnsignedInt(key, 8, 4);
        long d = TeaUtil.getUnsignedInt(key, 12, 4);
        // 这是算法的一些控制变量，为什么delta是0x9E3779B9呢？
        // 这个数是TEA算法的delta，实际是就是(sqr(5) - 1) * 2^31 (根号5，减1，再乘2的31次方)
        long sum = 0;
        long delta_ = delta;//32轮是0x57E89147;
        delta_ &= 0xFFFFFFFFL;
    ...
    private byte[] decipher(byte[] in, int offset) {
        ...
        // 我们用了long，这个long的前32位是全0的，我们通过这种方式模拟无符号整数，后面用到的long也都是一样的
        // 而且为了保证前32位为0，需要和0xFFFFFFFF做一下位与
        long y = TeaUtil.getUnsignedInt(in, offset, 4);
        long z = TeaUtil.getUnsignedInt(in, offset + 4, 4);
        long a = TeaUtil.getUnsignedInt(key, 0, 4);
        long b = TeaUtil.getUnsignedInt(key, 4, 4);
        long c = TeaUtil.getUnsignedInt(key, 8, 4);
        long d = TeaUtil.getUnsignedInt(key, 12, 4);
        ...
        //long sum = 0xE3779B90;
        long sum=loop==0x10?0xE3779B90:0xFD1228E0;
        sum &= 0xFFFFFFFFL;
        long delta_ = delta;
        delta_ &= 0xFFFFFFFFL;
    ...
    /*public String encrypt(String str,String key){
        byte[] bytes= encrypt(Converter.hexStr2ByteArray(str),Converter.hexStr2ByteArray(key));
        if(bytes!=null){
            return Converter.byteArray2HexStr(bytes);
        }else{
            return "";
        }
    ...
    public String decrypt(String str,String key){
        byte[] bytes=decrypt(Converter.hexStr2ByteArray(str),Converter.hexStr2ByteArray(key));
        if(bytes!=null){
            return Converter.byteArray2HexStr(bytes);
        }else{
            return "";
